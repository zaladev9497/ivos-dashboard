  ...
  let q = sb
    ...
    .select(
      `id, lead_id, journey_id, template_key, channel, scheduled_for,
       state, suppression_reason, attempts, last_attempt_at, error_message, context,
...
export async function getCadenceSteps(journeyType) {
  ...
  let q = sb
    .from('cadence_steps')
    .select('id, journey_type, trigger_event, track, template_key, channel, offset_value, offset_unit, offset_from, demo_offset_value, demo_offset_unit, cap_before_expiry_days, enabled, sort_order, notes, updated_at, updated_by')
    .order('trigger_event')
